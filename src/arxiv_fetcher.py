...
def _fetch_rss(categories: list) -> list:
    ...
    for cat in categories:
        url = f"{config.ARXIV_RSS_BASE}/{cat}"
        ...
        if feed.bozo and not feed.entries:
            logger.warning("RSS parse error for %s: %s", cat, feed.bozo_exception)
            continue
        ...
        all_papers.extend(seen.values())
        logger.info("Parsed %d unique papers from RSS for %s", len(seen), cat)
            ...
            for entry in feed.entries:
                ...
                if aid and aid in idx:
                    ...
                    if hasattr(entry, "authors") and entry.authors:
                        p["authors"] = [a.get("name", "") for a in entry.authors if a.get("name")]
